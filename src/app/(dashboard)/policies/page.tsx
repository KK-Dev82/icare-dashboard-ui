...
export default function ProductsPage() {
  ...
  const [notificationItem, setNotificationItem] = useState<Product | null>(null);
  ...
  const handleConfirmToggle = async () => {
    ...
    if (confirmItem.isPublish) {
      await productApi.unpublish(confirmItem.id);
    } else {
  ...
  return (
    <div>
        ...
        <div className="flex items-center justify-between pb-6 mb-6 border-b border-[#EAEAEA]">
          <div>
            ...
            <p className="mt-1 text-sm text-[#9CA3AF]">
              จัดการแผนประกันและผลิตภัณฑ์ที่แสดงบนหน้าเว็บ
            </p>
          </div>
          <button
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide py-3 px-4">ชื่อผลิตภัณฑ์</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      <div className="w-[72px] h-[72px] rounded-lg overflow-hidden bg-gray-100">
                        {item.mainImage ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-xs text-[#9CA3AF]">No Image</div>
                        )}
                      </div>
                    </td>
                    <td className="py-4 px-4 max-w-[280px]">
                      ...
                      {item.summary && (
                        <p className="text-xs text-[#9CA3AF] mt-0.5 line-clamp-1">{item.summary}</p>
                      )}
                    </td>
      ...
      <ConfirmModal
        open={!!confirmItem}
        title={confirmItem?.isPublish ? "ปิดการเผยแพร่" : "เผยแพร่ผลิตภัณฑ์"}
        message={
